...
export function StudentCompletionModal({
  isOpen,
  onClose,
  lessonId,
  lessonTitle,
  ...
  const load = useCallback(async () => {
    if (!isOpen || !lessonId || !programId) return;
    setLoading(true);
    try {
      // Fetch enrolled students for this program
      const res = await fetch(
        `${apiPrefix}/lessons/${lessonId}/student-completion?programId=${encodeURIComponent(programId)}`
      );
      if (res.ok) {
  ...
  }, [isOpen, lessonId, programId, apiPrefix]);
  ...
  async function save() {
    ...
    try {
      ...
      if (toMark.length > 0) {
        await fetch(`${apiPrefix}/lessons/${lessonId}/student-completion`, {
          method: "POST",
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Student completion — ${lessonTitle}`}>
      <div className="space-y-4">
        <p className="text-sm text-gray-500">
          Mark students who have completed this lesson. Changes are saved when you click <strong>Save</strong>.
        </p>
