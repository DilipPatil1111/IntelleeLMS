...
async function assertChapterAccess(sessionUserId: string, chapterId: string) {
  ...
  const can = await staffCanAccessProgram(sessionUserId, "PRINCIPAL", ch.subject.programId);
...
export async function PATCH(req: Request, { params }: { params: Promise<{ chapterId: string }> }) {
  ...
  const chapter = await db.programChapter.update({
    ...
    data: {
      ...
      ...(body.freePreviewLesson !== undefined ? { freePreviewLesson: body.freePreviewLesson } : {}),
      ...(body.isPrerequisite !== undefined ? { isPrerequisite: body.isPrerequisite } : {}),
      ...(body.enableDiscussions !== undefined ? { enableDiscussions: body.enableDiscussions } : {}),
    },
  });
