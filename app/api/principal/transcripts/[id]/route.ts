...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await db.transcript.findUnique({ where: { id }, select: { status: true } });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (existing.status === "PUBLISHED") {
    return NextResponse.json({ error: "Published transcripts cannot be edited" }, { status: 400 });
  }
  ...
  const subjectData = (subjects as Array<{
    id?: string; subjectCode?: string; subjectName: string; description?: string;
