...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const type = await db.programType.update({
      where: { id },
      data: {
        ...(name !== undefined && { name }),
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [pCount, aCount] = await Promise.all([
    db.program.count({ where: { programTypeId: id } }),
    db.programApplication.count({ where: { programTypeId: id } }),
  ]);
  ...
  await db.programType.delete({ where: { id } });
