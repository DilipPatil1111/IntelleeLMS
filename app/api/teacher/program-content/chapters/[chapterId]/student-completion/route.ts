...
async function assertAccess(
  ...
  chapterId: string,
  session: Parameters<typeof staffCanAccessProgram>[1],
) {
  ...
  const can = await staffCanAccessProgram(sessionUserId, session, chapter.subject.programId);
...
export async function GET(_req: Request, { params }: { params: Promise<{ chapterId: string }> }) {
  ...
  const a = await assertAccess(session.user.id, chapterId, session);
  ...
  // A student is "completed" for this chapter if ALL published non-QUIZ lessons are done
