...
export default async function PrincipalStudentsPage() {
  ...
  return (
    <>
      <PageHeader title="All Students" description="View performance and details of all students" />
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {students.map((s) => {
              const avg = s.attempts.length > 0 ? Math.round(s.attempts.reduce((sum, a) => sum + (a.percentage || 0), 0) / s.attempts.length) : 0;
              ...
              return (
                <tr key={s.id}>
                  ...
                  <td className="px-4 py-3"><Badge variant={avg >= 50 ? "success" : avg > 0 ? "danger" : "default"}>{avg}%</Badge></td>
                  <td className="px-4 py-3"><Badge variant={attRate >= 75 ? "success" : attRate >= 50 ? "warning" : "danger"}>{attRate}%</Badge></td>
                  <td className="px-4 py-3 text-sm text-gray-500">{s.attempts.length}</td>
                </tr>
              );
            })}
          </tbody>
