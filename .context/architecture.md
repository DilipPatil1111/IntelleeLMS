...
| Email | **Resend** via `lib/email.ts` (`sendEmail` helper) |
| File Storage | **Vercel Blob** (`@vercel/blob`) — all uploads (templates, docs, profile pictures) |
...
5. **File uploads** all go through `lib/file-upload.ts` → Vercel Blob Storage. Blob URLs are stored in the database. Local filesystem (`public/uploads/`) is no longer used.
...
- **Portal grants** system: `PortalAccess` model enables cross-role access (e.g., ADMIN can access student/teacher/principal portals).
...
- **Vercel Blob** for all file storage — no local disk writes, production-ready for serverless.
